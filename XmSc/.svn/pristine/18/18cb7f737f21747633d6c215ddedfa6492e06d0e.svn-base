...
import com.lsp.web.entity.FilesInfo;
...
@Namespace("/web")
@Results({@org.apache.struts2.convention.annotation.Result(name="reload", location="files.action", type="redirect")})
public class FilesAction extends GeneralAction<FilesInfo>
{
  ...
  private Long _id;
  private FilesInfo entity = new FilesInfo();
  ...
  public String execute() throws Exception {
    ...
    List<DBObject> list = this.basedao.getList(PubConstants.FILES_INFO, 
      whereMap, this.fypage, 10, sortMap);
    Struts2Utils.getRequest().setAttribute("filesList", list);
    this.fycount = this.basedao.getCount(PubConstants.FILES_INFO);
    ...
    Struts2Utils.getRequest().setAttribute("type", "files");
  ...
  public String save() throws Exception
  {
    try
    {
      if (this._id == null)
      {
        this._id = this.mongoSequence.currval(PubConstants.FILES_INFO);
      }
      ...
      entity.setType(new FileUtil().getFileExt(entity.getUrl())); 
      this.basedao.insert(PubConstants.FILES_INFO, this.entity);
  ...
  public String delete() throws Exception
  {
    ...
    {
      if (this._id != null) {
        DBObject emDbObject = this.basedao.getMessage(PubConstants.FILES_INFO, 
          this._id);
        ...
        this.basedao.delete(PubConstants.FILES_INFO, this._id);
  ...
  public void   savedetail(){
	  ...
	  try {
		  ...
		  filesInfo.set_id(mongoSequence.currval(PubConstants.FILES_INFO));
		  filesInfo.setUrl(url);
		  filesInfo.setTitle(title);
		  filesInfo.setCreatedate(new Date());
		  filesInfo.setPath(path);
		  filesInfo.setFolderid(Long.parseLong(folderid));
		  filesInfo.setToUser(toUser);
		  filesInfo.setSort(Integer.parseInt(sort));
		  filesInfo.setType(new FileUtil().getFileExt(url));
		  basedao.insert(PubConstants.FILES_INFO, filesInfo);
		  sub_map.put("state", 0);
    ...
    throws Exception
  {
    if (this._id != null)
    {
      DBObject emDbObject = this.basedao.getMessage(PubConstants.FILES_INFO, 
        this._id);
      this.entity = ((FilesInfo)UniObject.DBObjectToObject(emDbObject, 
        VideoInfo.class));
    } else {
      this.entity = new FilesInfo();
    }
  }

  public FilesInfo getModel()
  {
    return this.entity;
  }
}