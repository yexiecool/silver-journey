...
var solarTerm = new Array("&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "清明", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp", "&nbsp")
var sTermInfo = new Array(0, 21208, 42467, 63836, 85337, 107014, 128867, 150921, 173149, 195551, 218072, 240693, 263343, 285989, 308563, 331033, 353350, 375494, 397447, 419210, 440795, 462224, 483532, 504758)
//var nStr1 = new Array('日', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十')
var nStr1 = new Array('&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp', '&nbsp')
//var nStr2 = new Array('初', '十', '廿', '卅', ' ')
var nStr2 = new Array('&nbsp', '&nbsp', '&nbsp', '&nbsp', ' ')
...
function calendar(y, m) {
    ...
    if (m == 3) this[tmp1].color = '#90b721'; //清明颜色
    ...
    for (i  in  sFtv)
        if (sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
            if (Number(RegExp.$1) == (m + 1)) {
                ...
                if (RegExp.$3 == '*')  this[Number(RegExp.$2) - 1].color = '#90b721'
            }
        ...
        if (lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
            ...
            if (tmp1 >= 0 && tmp1 < n) {
                ...
                if (tmp2 >= 0 && tmp2 < this.length) {
                    ...
                    if (RegExp.$3 == '*')  this[tmp2].color = '#90b721'
                }
            }
        }
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            if (cld[sD].lDay == 1)  //显示农历月
                //lObj.innerHTML = '<b>' + (cld[sD].isLeap ? '闰' : '') + cld[sD].lMonth + '月' + (monthDays(cld[sD].lYear, cld[sD].lMonth) == 29 ? ' ' : ' ') + '</b>';
            	lObj.innerHTML = '<b>' + (monthDays(cld[sD].lYear, cld[sD].lMonth) == 29 ? ' ' : ' ') + '</b>';
            else  //显示农历日
            ...
            if (s.length > 0) {  //农历节日
                ...
                if (s.length > 0) {
                    if (s.length > 8)  s = s.substr(0, 5) + '...';
                    s = (s == ' ') ? s.fontcolor('#90b721') : s.fontcolor('#90b721');
                }
                else {  //廿四节气
                    s = cld[sD].solarTerms;
...
function mOck(thisObj, v){
	...
	if(dayColor&&dayColor.value=='#aaa'&&getH(dayF)){
		 lx = '1';
	}
	dayJson = '{holiday:'+nian+addZ(yue)+addZ(day)+',lx:'+lx+'}';
	getGdvalue(thisObj.id);
	for (var i =0; i <42; i++) { 
	...
	if(onoff == '0'){
   	...
   	thisObj.setAttribute("class", "noday");
