    ...
    <script type="text/javascript">
        ...
        function del(id) {
            if (confirm('确实要删除吗?')) {
                ...
                location.href = "${contextPath}/set/price!delete.action?_id="
                + id;
            }
        }
        function add() {
        ...
        function upd(id) {
            ...
            $.post('${ctx}/set/price!upd.action', submitData, function (json) {
                $('#_id').val(json._id);
...
<section>
    ...
    <div class="mainpanel">
        ...
        <form id="custinfoForm" name="custinfoForm" method="post" action="${contextPath}/set/price.action?">
            <div class="pageheader">
                <h2><i class="fa fa-user"></i>系统配置<span>价格管理</span></h2>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-primary table-action mb30">
                                ...
                                <tr>
                                    ...
                                    <th class="th5 table-action">分类</th>
                                    <th class="th5 table-action">价格</th>
                                    <th class="th5 table-action">操作</th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach items="${priceList}" var="bean">
                                    <tr>
                                        <td>${bean.title}</td>
                                        <td>
                                            <c:if test="${bean.type=='bbs_adv'}">
                                                论坛广告
                                            </c:if>
                                        </td>
                                        <td>${bean.price}</td>
            ...
            <div class=" bg-bai border-radius3 overflow-hidden">
                <div class="overflow-hidden line-height40 bg-bai line-bottom">
                    <div class="hang50 pull-left zi-hui-tq">
                        <i class="weight500 size14 pl-10 line-height50">价格设置</i>
                    </div>
                ...
                <form id="inscxForm" action="${ctx }/set/price!save.action?fypage=${fypage}"
