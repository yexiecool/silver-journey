...
//        }
		...
		//统计总金额
		Double sum = 0.0;
		List<DBObject> listsum = baseDao.getList(PubConstants.INTEGRALLL_INFO, whereMap, sortMap);
		if(listsum.size()>0){
	 ...
	 * @throws Exception
	 */
	public void integeralLLBfromexp() throws Exception {
		...
		for (DBObject dbObject : list) {
			if (dbObject.get("fromUserid") != null) {			 
			 ...
			 dbObject.put("vipno", wwzService.getVipNo(dbObject.get("fromUserid").toString()));
		...
		String[] header = { "会员编号", "状态","金额", "类型", "时间" };
		String[] body = { "vipno", "state", "value","type", "insDate"};
		String newtime = "integeralll" + ".xls";
	...
	public String profit(){
		HashMap<String, Object> sortMap = new HashMap<String, Object>();
		HashMap<String, Object> whereMap = new HashMap<String, Object>();
		HashMap<String, Object> lowerwhereMap = new HashMap<String, Object>();
		if (StringUtils.isNotEmpty(Struts2Utils.getParameter("fypage"))) {
			fypage = Integer.parseInt(Struts2Utils.getParameter("fypage"));
		}
		whereMap.put("renumber", 0);
		...
		for (DBObject dbObject : list) {
			...
			dbObject.put("xsyj",getResults(number));
			lowerwhereMap.put("renumber", number);
			long lowerCount = baseDao.getCount(PubConstants.USER_INFO,whereMap);
			if(lowerCount>0) {
				dbObject.put("lower", 1);
			}else {
				dbObject.put("lower", 0);
			}
		}
	 ...
	 * 下级用户集合
	 */
	public void findLowerUser() {
		HashMap<String, Object> sub_map = new HashMap<>();
		HashMap<String, Object> sortMap = new HashMap<String, Object>();
		HashMap<String, Object> whereMap = new HashMap<String, Object>();
		HashMap<String, Object> lowerwhereMap = new HashMap<String, Object>();
		whereMap.put("renumber", Struts2Utils.getParameter("renumber"));
		List<DBObject> list = baseDao.getList(PubConstants.USER_INFO, whereMap, sortMap);
		for (DBObject dbObject : list) {
			Long number = Long.parseLong( dbObject.get("no").toString());//用户的编号
			dbObject.put("xsyj",getResults(number));
			lowerwhereMap.put("renumber", number);
			long lowerCount = baseDao.getCount(PubConstants.USER_INFO,whereMap);
			if(lowerCount>0) {
				dbObject.put("lowerUser", 1);
			}else {
				dbObject.put("lowerUser", 0);
			}
		}
