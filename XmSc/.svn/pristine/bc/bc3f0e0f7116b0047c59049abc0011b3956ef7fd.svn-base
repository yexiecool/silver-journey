...
//		}
		...
		String hyq_account = SysConfig.getProperty("hyq_account");
		String dzq_account = SysConfig.getProperty("dzq_account");
		String tyq_account = SysConfig.getProperty("tyq_account");
		System.out.println(hyq_account);
		System.out.println(dzq_account);
		System.out.println(tyq_account);
		...
		}else if(custid.equals(hyq_account) || custid.equals(dzq_account) || custid.equals(tyq_account)){
			Struts2Utils.getRequest().setAttribute("accounttype", 0);
			BasicDBList dblist = new BasicDBList();
			//下单支出
//			dblist.add(new BasicDBObject("type", "shop_jfdh"));
			//订单收益
			dblist.add(new BasicDBObject("type", "shop_jfsr"));
			//交易转入
		...
		for (DBObject dbObject : list) {
			...
			dbObject.put("vip_no",wwzService.getVipNo(String.valueOf(dbObject.get("fromUserid"))));
			dbObject.put("level", wwzService.getWXuserVipNo(wwzService.getVipNo(String.valueOf(dbObject.get("fromUserid")))).get("agentLevel"));
		}
	...
	public String panpan() throws Exception {
		...
		if (StringUtils.isNotEmpty(type)) {
			...
			//小区业绩赠送
			dblist.add(new BasicDBObject("type", "recommend_give"));
			//消费兑换
			dblist.add(new BasicDBObject("type", "shop_exchange"));
			// or判断
			whereMap.put("$or", dblist);
		}
		String sel_insdate = Struts2Utils.getParameter("sel_insdate");
		...
		for (DBObject dbObject : list) {
			...
			dbObject.put("level", wwzService.getWXuserVipNo(wwzService.getVipNo(dbObject.get("fromUserid").toString())).get("agentLevel"));
		}
		//统计总金额
		Double sum = 0.0;
		List<DBObject> listsum = baseDao.getList(PubConstants.INTEGRAL_INFO, whereMap, sortMap);
		if(listsum.size()>0){
			for(int i=0;i<listsum.size();i++){
				sum+=Double.valueOf(listsum.get(i).get("value").toString());
			}
...
*/		String state = Struts2Utils.getParameter("state");
	...
	public String profit() throws Exception {
		...
		String type = Struts2Utils.getParameter("sel_type");
		if (StringUtils.isNotEmpty(type)) {
			if("pada_jian".equals(type)){
				whereMap.put("state", 1);// 支出
			}else{
				whereMap.put("state", 0);// 收入
			}
			whereMap.put("type", type);
			Struts2Utils.getRequest().setAttribute("type", type);
		}else{
...
//		}
	...
	public void ajaxweb() throws Exception {
		...
		dblist.add(new BasicDBObject("type", "jyscz"));
		dblist.add(new BasicDBObject("type", "shop_tx")); 
		dblist.add(new BasicDBObject("type", "returngood_add"));
		dblist.add(new BasicDBObject("type", "returngood_jian")); 
		// or判断
		whereMap.put("$or", dblist);
	 ...
	 * @throws Exception
	 */
	public void integeralpanpanfromexp() throws Exception {
		HashMap<String, Object> sortMap = new HashMap<String, Object>();
		HashMap<String, Object> whereMap = new HashMap<String, Object>();  
		String  comid=Struts2Utils.getParameter("comid");
		if(StringUtils.isNotEmpty(comid)){
		...
		String[] header = { "会员号", "状态","金额", "类型", "时间" };
		String[] body = { "vip_no", "state", "value","type", "insDate"};
		String newtime = "integeral-panpan" + ".xls";
	...
	public String extract(){
		...
		String remark = Struts2Utils.getParameter("remark");
		if(StringUtils.isNotEmpty(Struts2Utils.getParameter("remark"))){
			Pattern pattern = Pattern.compile("^.*" + remark + ".*$",
					Pattern.CASE_INSENSITIVE);
			whereMap.put("remark", pattern);
			Struts2Utils.getRequest().setAttribute("remark", remark);
		}
		List<DBObject> list = baseDao.getList(PubConstants.INTEGRAL_INFO, whereMap, fypage, 10, sortMap, backMap);
		fycount = baseDao.getCount(PubConstants.INTEGRAL_INFO,whereMap);
		Struts2Utils.getRequest().setAttribute("fycount", fycount);
