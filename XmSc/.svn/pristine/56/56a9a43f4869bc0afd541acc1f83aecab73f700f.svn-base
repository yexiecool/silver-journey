	...
	public void saveMiner() throws Exception{
		...
		if(db != null){
			if(db.get("price")!=null){  
					boolean fag=wwzService.deljf(db.get("price").toString(), fromUserid, "shop_exchange", custid, 1, 1, 1);//兑换矿机
					if(fag) {
						...
						int  agentLevel =100 ; //矿机会员类型 100-微型矿机   101-小型矿机 102-中型矿机 103-大型矿机
						double money =0.0;
						switch ( miner.getPrice().intValue()) {
							case 300:
								money = miner.getPrice();
								agentLevel=100;
								break;
							case 3000:
								money = miner.getPrice();
								...
								break;
							case 5000:
								money = miner.getPrice()*1.5;
								agentLevel =102;
								break;
							case 10000:
								money = miner.getPrice()*2;
								agentLevel=103;
								break;
						}
						prostore.setMoney(money);
						prostore.setTime(miner.getTime());
						prostore.setEnddate(DateUtil.addDay(new Date(),miner.getTime()));
						prostore.setType("shop_exchange");
						prostore.setState(0);
						prostore.setKj(db);
						baseDao.insert(PubConstants.INTEGRAL_PROSTORE, prostore);
						//根据会用户Id获取用户信息
						DBObject dbObject = wwzService.getCustUser(fromUserid);
						//设置用户的角色
						if(dbObject!=null) {
							UserInfo obj = (UserInfo) UniObject.DBObjectToObject(dbObject, UserInfo.class);
							if(obj.getAgentLevel() == 0  ) {
								obj.setAgentLevel(agentLevel);
							}else if(obj.getAgentLevel() < agentLevel  && obj.getAgentLevel()  > 99) { //不是省市区报 且当前购买矿机登记大于之前的登记
								obj.setAgentLevel(agentLevel);
							}
						...
						//计算分享奖到佣金账户
						wwzService.addShare(fromUserid,String.valueOf(miner.getPrice()));
						
						//共创奖
						wwzService.together(dbObject.get("number").toString(),miner.getPrice());
						//计算见点奖
						int price =  (int) Math.ceil( miner.getPrice() )  ;
						wwzService.sharedReward(price,dbObject.get("number").toString(),fromUserid);
						sub_map.put("state", 0);//积分不足
					}else {
						sub_map.put("state", 2);//积分不足
					}
			}else{
				sub_map.put("state", 3);//积分不足
			}
		}
		String json = JSONArray.fromObject(sub_map).toString();
		Struts2Utils.renderJson(json.substring(1, json.length() - 1), new String[0]);
	...
	public  void myMiner() throws Exception{
		...
		if(list.size()>0){
			for (DBObject dbObject : list) {
				if(null != dbObject.get("enddate") && !"".equals(dbObject.get("enddate"))) {
					if(null != dbObject.get("kj") && !"".equals(dbObject.get("kj"))) {
						DBObject kjDb = (DBObject)dbObject.get("kj");
						double price = (Double)kjDb.get("price");
						switch ((int)price) {
						case 5000:
							kjDb.put("price", price * 1.5);
							dbObject.put("kj", kjDb);
							break;
						case 10000:
							kjDb.put("price", price * 2);
							dbObject.put("kj", kjDb);
							break;
						default:
							break;
	...
	/***
	  ...
	  public String ownperson(){
		  ...
		  if(StringUtils.isNotEmpty(fromUserid)){
			  ...
			  if (wwzService.getJfOBJ(SysConfig.getProperty("custid"), fromUserid)!=null) {
				  Struts2Utils.getRequest().setAttribute("yongjin",wwzService.getJfOBJ(SysConfig.getProperty("custid"), fromUserid).get("yjvalue") );
			  }
		  ...
		  if( dbObject != null){
			  if(dbObject.get("kjvalue") != null){
				  if(dbObject.get("kjjzvalue") != null){
					  String kjjzvalue = BaseDecimal.division(dbObject.get("kjjzvalue").toString(), ppb,10);
		  ...
		  //只有代理显示多少天没有提币
		  if(0 != (Integer)wxUser.get("agentLevel")) {
			  HashMap<String, Object> sortMap = new HashMap<String, Object>();
			  sortMap.put("createdate", Integer.valueOf(-1));
			  List<DBObject> list = baseDao.getList(PubConstants.INTE_TBJL, whereMap, sortMap);
			  if(list.size()>0 ) {
				  DBObject dbObjec1 = list.get(0);
				  String perdate=dbObjec1.get("createdate").toString();
				  Date oldDate = DateFormat.getFormat(perdate);
				  Date newDate = new Date();
				  long time = DateUtil.getTimeDifference(newDate, oldDate);
				  Struts2Utils.getRequest().setAttribute("tbjlday", time/(24*60*60*1000));
			  }
		  }
		  ...
		  //查询用户未分配的关系
		  int count =0; //未分配的人数
		  String  number = wwzService.getVipNo(fromUserid);
		  if(number!="") {
		  whereMap.clear();
		  whereMap.put("renumber", Integer.valueOf(number));
		  BasicDBList   dblist=new BasicDBList(); 
			dblist.add(new BasicDBObject("recommend",0));
			dblist.add(new BasicDBObject("recommend",null));
			whereMap.put("$or", dblist); 
		  HashMap<String, Object> sortMap = new HashMap<String, Object>(); 
		  List<DBObject> userlist= baseDao.getList(PubConstants.USER_INFO, whereMap,sortMap);
		  if(userlist!=null) {
			  count =userlist.size();
			  System.out.println("------------------未分配的人数："+count);
		  }
		  }
		  Struts2Utils.getRequest().setAttribute("numbercount", count);
	  ...
	  public String szjy(){
		  ...
		  //积分 
		  if (wwzService.getJfOBJ(SysConfig.getProperty("custid"), fromUserid)!=null) {
			  DBObject db1 = baseDao.getMessage(PubConstants.USER_INFO, fromUserid);
				if(db1.get("status")==null || db1.get("status").equals("0") || db1.get("status").equals("")){
					Struts2Utils.getRequest().setAttribute("status", 0);
				}else{
					Struts2Utils.getRequest().setAttribute("status", 1);
				}
			  Struts2Utils.getRequest().setAttribute("jf",wwzService.getJfOBJ(SysConfig.getProperty("custid"), fromUserid));
		  } 
		  return "szjy";
	  ...
	  public String wfp(){
		  ...
		  List<DBObject> userlist = baseDao.getList(PubConstants.USER_INFO, whereMap, fypage, 50, sortMap); 
	  ...
	  /***
	    ...
	    public void   transfer() {
	    	...
	    	Map<String,Object>sub_map = new HashMap<>();
		  	sub_map.put("state", 1);
	    	String toid=Struts2Utils.getParameter("toid").trim();
	    		...
	    		Code code=GetAllFunc.telcode.get(tel);
 	  	   	    if (code!=null&&code.getCode().equals(yzm)) {
 	  	   	    	if(DateUtil.checkbig(DateUtil.addMinute(code.getCreatedate(),10))) {
	 	  	   	    		//根据用户的会员号查询用户的id
		 	  	 	    	HashMap<String, Object> whereMap = new HashMap<String, Object>();
		 	  	 	    	whereMap.put("number",Long.parseLong( toid));
		 	  	 			
		 	  	 			List<DBObject> list1=baseDao.getList(PubConstants.USER_INFO, whereMap,null);
		 	  	 			if(list1.size() > 0) {
		 	  	 				DBObject object= list1.get(0);
		 	  	 				String touserid=object.get("_id").toString();
		 	  	 	    	...
		 	  	 	    	if(StringUtils.isNotEmpty(price)) {
		 	  	 		    	...
		 	  	 		    	transferOrder.setCustid(SysConfig.getProperty("custid"));
		 	  	 		    	transferOrder.setFromid(fromUserid);
		 	  	 		    	transferOrder.setPrice(Double.parseDouble(price));
		 	  	 		    	//transferOrder.setPrice(Integer.parseInt(price));
		 	  	 		    	transferOrder.setToid(touserid);
		 	  	 		    	...
		 	  	 		    	//开始转账 
		 	  	 		    	if(wwzService.deljf(price, fromUserid, "pada_jian", SysConfig.getProperty("custid"), 0, 1, 0)) {  
		 	  	 		    		//String pro=BaseDecimal.subtract(price, BaseDecimal.multiplication(BaseDecimal.division(price, "100",6),"0.2"));
		 	  	 		    		if(wwzService.addjf(price, touserid, "pada_add", SysConfig.getProperty("custid"), 0, 1, 0)) {
		 	  	 		    			sub_map.put("state", 0);
 	  	   	    	...
 	  	   	    	sub_map.put("state", 6);//验证码超时！
 	  	   	    	}
 	  	   	    }else {
 	  	   	    	sub_map.put("state", 5);//验证码有误！
 	  	   	    }
	    	}
	    	String json = JSONArray.fromObject(sub_map).toString();
	  		Struts2Utils.renderJson(json.substring(1, json.length() - 1), new String[0]);
	    ...
	    public void   wdpassword() throws Exception{
	    	...
	    	Map<String,Object>sub_map = new HashMap<>();
		  	...
		  	if(dbObject !=null){
		  		if(dbObject.get("paypassword")!=null && dbObject.get("salt") != null &&  !dbObject.get("salt").equals("")){
		  						...
		  						if(ir.getKjlx()>=0){
		  					    	...
		  					    	if(wwzService.delyfjf(price, fromUserid, "kj_tx", SysConfig.getProperty("custid"),BaseDecimal.multiplication(price, wwzService.getPPBSprice()+""),orderno)) {
		  					            ...
		  					            if(obj.getString("code").equals("1000")) {
		  							    	...
		  							    	wwzService.addyfjf(price, fromUserid, "kj_txfh", SysConfig.getProperty("custid"),3,null, BaseDecimal.multiplication(price,ppbprice),ppbprice);
		  							    	sub_map.put("state", 3);
		  					            }
		  					    	}else {
		  					    		//余额不足
		 ...
		 * 获取fromuserid
		 */
		public void ajaxfromuserid()
		{
			getLscode();
			Map<String, Object> sub_map = new HashMap<>();
			sub_map.put("state", 0);
			HashMap<String, Object> whereMap = new HashMap<String, Object>();
			String toUser=Struts2Utils.getParameter("toid");
			whereMap = new HashMap<String, Object>();
		    whereMap.put("no", toUser);
		    DBObject user = baseDao.getMessage(PubConstants.USER_INFO, whereMap);
 		     if(user != null){
 		    	String fromuserid=wwzService.createcode(user.get("_id").toString()); 
                sub_map.put("state", 1);//登陆成功
                sub_map.put("fromuserid", fromuserid);
 		     }
			String json = JSONArray.fromObject(sub_map).toString();
			Struts2Utils.renderJson(json.substring(1, json.length() - 1), new String[0]);
