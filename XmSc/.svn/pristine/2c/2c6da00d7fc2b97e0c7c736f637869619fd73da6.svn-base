/*
 * @(#)calendar.js Create on 2009-2-17 下午07:59:39      
 * 
 * Copyright 2008 DTMobile, Inc. All rights reserved.
 */
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var cbx  = this.getAttribute(source, "cbx");
  var hint  = this.getAttribute(source, "hint");
  var sid   = sourceIndex.substr(sourceIndex.indexOf(d) + d.length);
  var globalPath="";
  if(parentId==0){
  	globalPath=sourceIndex;
  }else{
  	var parentName=this.node[parentId].globalPath;
    parentName=parentName.substr(0,parentName.lastIndexOf("_")+1);
    globalPath=parentName+sourceIndex;
  }
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  HTML += "<IMG "+
    ...
    if(this.isCbx) HTML+="<input type=checkbox "+node.cbx+" name='"+node.globalPath+"' id='cbx_"+id+"' onclick='"+this.name+".cbxClick(this)'>";
    HTML+="<A "+
...
MzTreeView.prototype.cbxClick=function(obj){
	//alert(obj.name);
	...
	var p=o.length;
	for(var i=0;i<p;i++){
		if(o[i].name.indexOf(objName+"_")==0) {
			o[i].checked=objCheck;
		}
		if(o[i].name.indexOf(objName)==0){
			o[i].checked=obj.checked;
		}
	}
	if(obj.checked){
		this.checkNode(objName);
	}else{
		this.unCheckNode(id);
			this.unCheckNode(id,o);
	}
};
MzTreeView.prototype.checkNode=function(id){
	if(id.lastIndexOf("_")<=1) return;	
	var name=id.substr(id,id.lastIndexOf("_"));
	var tar=document.getElementsByName(name);
	if(tar.length==1){
		tar[0].checked=true;
		this.checkNode(tar[0].name);
	}
};
MzTreeView.prototype.unCheckNode=function(eId){
	//alert(eId);
	if(eId==0) return;
	var e=this.node[eId].parentId;
	var tmpNode=this.node[e];
	var a = tmpNode.childNodes;
	var tmpPath="";
  for(var i=0; i<a.length; i++){
  	tmpPath=a[i].globalPath;
  	var tar=document.getElementsByName(tmpPath);
		if(tar.length==1){
			if(tar[0].checked){
				return;
			}
  ...
  var tmpTar=document.getElementsByName(tmpNode.globalPath);
	if(tmpTar.length==1){
		tmpTar[0].checked=false;
		this.unCheckNode(e);
	}
