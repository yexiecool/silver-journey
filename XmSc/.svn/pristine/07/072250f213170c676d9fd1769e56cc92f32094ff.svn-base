...
<head>
	...
	<body style="background:url('${ctx}/xmMobile/images/kjbg2.jpg') repeat-y top center; background-size:100% 100%;">
		...
		<style>
		.zichan p span{color:#7cc3df;  float:left; text-align: center !important;}
		.zichan span.fir{ width:25%;}
		.zichan span.two{ width:55%; text-align: left;}
		.zichan span.thr{ width:20%;}
		</style>
		...
		<div class="zichan" style="padding-top:50px;">
			<p style="border-top:none;"><span class="fir">会员编号</span><span class="two">昵称</span><span class="thr">操作</span></p>
			<div  style="clear: both;"></div>
			<c:forEach items="${userlist}" var="obj">
				<input type="hidden" value="${obj.renumber}"> 
				<p><span class="fir">${obj.number}</span>
				<span  class="two">			
					${obj.nickname}
				</span>
