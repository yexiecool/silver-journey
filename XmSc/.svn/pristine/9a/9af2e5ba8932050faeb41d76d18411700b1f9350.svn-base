...
//给map绑定mousemove事件
    ...
    qq.maps.event.addListener(map,"mousedown",function(e){
       //$("#latLng").val(e.latLng.toString());
        var latlngStr = e.latLng.toString().split(",",2);
      
        $("#page_lng").val(latlngStr[1].trim());
        $("#page_lat").val(latlngStr[0]);
    });
    //调用地址解析类
...
function codeLatLng() {
    ...
    var lat = parseFloat($("#page_lat").val());
    var lng = parseFloat($("#page_lng").val());
...
<div class="col-sm-7">
<input  class="form-control" id="adddress" placeholder="请输入城市" type="text">
</div>
<div class="col-sm-2">
