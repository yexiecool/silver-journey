...
var editor=CKEDITOR.replace('context');
	...
	function del(id) {
		if (confirm('确实要删除吗?')) {
			location.href = "${ctx}/shop/shopjoin!delete.action?type=${type}&fypage=${fypage}&_id="
					+ id;
		}
	}
	function add() {
	...
	function upd(id) {
		...
		$.post('${ctx}/shop/shopjoin!upd.action', submitData, function(json) {
...
<body>
	<section>
		...
		<div class="mainpanel">
			...
			<form id="shopjoinForm" name="custinfoForm" method="post"
				action="${contextPath}/shop/shopjoin.action">
				<div class="pageheader">
					<h2>
						<i class="fa fa-user"></i> 微官网 <span>线下招商会</span>
					</h2>
							...
							<div class="form-group col-sm-1d pull-right">
								<sec:authorize ifAnyGranted="ROLE_159">
									<button onclick="add()" type="button"
										...
										添加招商会<i class="fa fa-align-center"></i>
									</button>
								</sec:authorize>
							</div>
						...
						<div class="col-md-12">
							<div class="table-responsive">
								<table
									...
									<tbody>
										<c:forEach items="${joinlist}" var="bean">
											<tr>
												<td>${bean.sort}</td>
												<td>${bean.title}</td> 
		...
		<div style="padding-top: 2%">
			<div class="pl-10 pr-10 maring-a cmp500"
				...
				<div class=" bg-bai border-radius3 overflow-hidden">
					...
					<form id="addshopjoinForm"
						action="${ctx}/shop/shopjoin!save.action?fypage=${fypage}"
