$(document).ready(function(){
  // 字符最小长度验证（一个中文字符长度为2）
    ...
    }, $.validator.format("长度不能小于{0}字节!"));
   
    // 字符最大长度验证（一个中文字符长度为2）
    jQuery.validator.addMethod("stringMaxLength", function(value, element, param) {
     var length = value.length;
     for ( var i = 0; i < value.length; i++) {
      if (value.charCodeAt(i) > 127 || value.charCodeAt(i) == 10) {
       length++;
      }
     }
     return this.optional(element) || (length <= param);
    }, $.validator.format("长度不能大于{0}字节!"));
 ...
 // 字符验证       
  ...
  // 字符验证
    ...
    jQuery.validator.addMethod("greaterTo", function(value, element, param) {
     return value >= $(param).val();
    }, $.validator.format("第二次输入值小于第一次输入值"));
 // 验证值不允许与特定值等于
 ...
 // 汉字
  ...
  /*
  jQuery.validator.addMethod("autoValidate",function(value, element, param){
	  var ids = param.split(" ");
...
function timeInput_onChange(param){
	var ids = param.split(" ");
	for(var i = 0; i < ids.length; i++){
	  	$(ids[i]).focusout();
	}
