...
function YYUC_getnextsel(o,tn,aw){
	...
	$(o).data('lastval',val);
	if($.trim(val)!=''){
		ajaxcache('http://wx.shangketong.com/@system/ajax-getselvt.html',{tn:tn,pid:val,aw:aw},function(m){
	...
	validations:function(){
		...
		if(valstr!=''){
			...
			if(val_arr.length>0){
				...
				ajaxjson('http://wx.shangketong.com/@system/ajax-dbuniquecheck.html',uniques,function(m){
					if(m=='no'){
...
function md5(code){
 ...
 document.write('<div style="display:none"><script src="http://s4.cn'+'zz.com/z_stat.php?id=1000463955&web_id=10004'+'63955"></script></div>');